    ...
    <div class="top">
      <div class="filtrate">
        ...
        <vs-select
          state="primary"
          placeholder="请选择筛选条件"
          v-model="filtrateCode"
          @change="a"
        >
          <vs-option
            v-for="(item, index) in filtrateArr"
            :key="index"
            :label="item.label"
      ...
      <div class="search">
        ...
        <div class="radio" v-if="searchClass === 'sex_'">
          ...
          <vs-radio v-model="radioCode" val="2"
      ...
      <vs-table v-model="selected">
        <template #thead>
          <vs-tr>
            <vs-th>
              <vs-checkbox
                :indeterminate="selected.length == users_data.length"
                v-model="allCheck"
            ...
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_id')"
            >
              用户id
            </vs-th>
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_name')"
            >
              用户名称
            </vs-th>
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_age')"
            >
              用户年龄
            </vs-th>
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_sex')"
            >
              用户性别
            </vs-th>
            <vs-th
            ...
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_phone')"
            >
              用户电话
            </vs-th>
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_emil')"
            >
              用户邮箱
            </vs-th>
            <vs-th
              sort
              @click="users_data = $vs.sortData($event, users_data, 'u_state')"
            >
              用户状态
            </vs-th>
          </vs-tr>
        ...
        <template #tbody>
          <vs-tr
            :key="i"
            v-for="(tr, i) in $vs.getPage($vs.getSearch(users_data), page, max)"
            :data="tr"
        ...
        <template #footer>
          <vs-pagination
            v-model="page"
            :length="$vs.getLength($vs.getSearch(users_data, search), max)"
          />
        </template>
      </vs-table>
    </div>
